...
The monopoles associated to the breaking of $G$ to its Cartan were analyzed in many places, see e.g.~\cite{Gauntlett:1996cw}.\footnote{%
There were many competing papers from the same period concerning the application to Seiberg-Witten;
the analysis of the monopoles itself is much older and I should find proper original references for it too.
}
The upshot is that you can embed the standard spherically-symmetric 't Hooft-Polyakov monopole using any simple root into $G$ and have a monopole solution without additional bosonic moduli. 

So we consider a monopole with the charge $(1,1,0,\ldots,0)$.
This uses $su(2)_1$ within $so(4)\simeq su(2)_1\times su(2)_2$ within $so(2n_c)\subset so(4)\times so(4n_c-4)$.

Since the fermions are in the vector representation of $so(2n_c)$,
the charged part under $su(2)_1$ is of charge 1 and in the doublet of $su(2)_2$.
Therefore it gives rise to fermionic zero modes transforming in \begin{equation}
V_{2} \otimes R_{2n_f} 
\end{equation} where $V_2$ is the doublet of $su(2)_2$, 
...
In either case, the $-1$ in $SO(2n_c)\times USp(2n_f)$ acts nontrivially,
...
\end{equation}
Again,
it is in the `adjoint' charge under the $usp(4)$ flavor symmetry.
...
\begin{equation}
	\begin{array}{c}
		...
		\begin{array}{c|c:cccccccccccc}
			...
			0 & \bZ &  & \bZ_4 &  & \red{\fbox{\black{$\bZ^{\oplus 2}$}}} & \Blue{\fbox{\black{$\bZ_2^{\oplus 2}$}}}\cellcolor{lightyellow} & \green{\fbox{\black{$\ast$}}}\\
			\hline
			& 0 & 1 & 2 & 3 & 4 & 5 & 6 \\
		\end{array}
	\end{array}
...
\begin{equation}
	\begin{array}{c|c:cccccccccccc}
		...
		2  & \bZ_2 &&& \cellcolor{lightyellow} & \ast & \ast & \ast\\
		1  & \bZ_2 &&&& \cellcolor{lightyellow} & \ast & \ast\\
		0 & \bZ &  & \bZ_4 &  & \bZ^{\oplus 2} & \bZ_2\cellcolor{lightyellow} & \ast\\
		\hline
		& 0 & 1 & 2 & 3 & 4 & 5 & 6 \\
	\end{array}
\end{equation}
...
%\subsection{Adams SS}
%The above analysis might be complemented by Adams SS.
%According to our naive guess,
%the module $\widetilde H^\ast(B\left(\tfrac{SO(4n'_c+2)\times SU(2n_f)}{\bZ_2}\right);\bZ_2)_{\leq 5}$ consists of
%\begin{equation}
%	\begin{tikzpicture}[thick,baseline=(m-2-4.south)]
%		\matrix (m) [
%			matrix of math nodes,
...
%		]{
%			& \colorB{\bullet} \\
%			& \colorB{\bullet} & \colorC{\bullet}\\
%		    & \colorB{\bullet} & \\
%			\colorA{\bullet} && \colorC{\bullet}\\
%			& \colorB{\bullet} & \\
%			\colorA{\bullet} &&\\
%		};
%		\draw[colorA] (m-4-1.center) to [out=200, in=160] (m-6-1.center);
%		%
%		\node[colorA, right=4pt] at (m-6-1) {$a_2$};
%		\node[colorA, right=4pt] at (m-4-1) {$(a_2)^2$};
%		%
%		\draw[colorB] (m-5-2.center) to [out=160, in=200] (m-3-2.center);
%		\draw[colorB] (m-3-2.center) to (m-2-2.center);
%		\draw[colorB] (m-3-2.center) to [out=160, in=200] (m-1-2.center);
%		%
%		\node[colorB, right=4pt] at (m-5-2) {$y'(1)$};
%		\node[colorB, right=4pt] at (m-3-2) {$y'(2)$};
%		\node[colorB, right=4pt] at (m-2-2) {$y'(1)^2$};
%		\node[colorB, right=4pt] at (m-1-2) {$y'(3)$};
%		%
%		\draw[colorC] (m-2-3.center) to [out=200, in=160] (m-4-3.center);
%		%
%		\node[colorC, right=4pt] at (m-4-3) {$c_2$};
%		\node[colorC, right=4pt] at (m-2-3) {$c_3$};
%	\end{tikzpicture}
%\end{equation}
%This leads to the following Adams chart:
%\begin{center}
