...
and therefore the dynamical monopole we need to analyze has the charge $(2,0,\ldots,0)$.

Since the fermions are in the vector representation of $so(2n_c)$,
they are charge 2 under the $so(2)$, and should give rise to fermionic zero modes transforming in \begin{equation}
V_{2} \otimes R_{2n_f} 
\end{equation} where $V_2$ is the doublet of spacetime $so(3)$, 
...
so it has the `vector' charge under $usp(2)$ flavor symmetry.
Therefore the 1-form symmetry is extended.

When $n_f=2$, there are $8$ Majorana fermions, 
and spatial $su(2)$ and flavor $usp(4)$ are embedded into $so(8)$ via \begin{equation}
...
\end{equation}
Again taking the spacetime scalar part, it is in the `adjoint' charge under the $usp(4)$ flavor symmetry.
