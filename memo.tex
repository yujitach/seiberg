...
\section{Bockstein square for non-closed cochains}
By definition, the variation of the Pontrjagin square $\fP : H^\bullet(-;\bZ_{2m}) \to H^{2\bullet}(-;\bZ_{4m})$ term is
\begin{equation}
	\label{deltaBockstein}
	\begin{array}{ccl}
		\delta \left(\dfrac{1}{4m}\fP(x)\right)
		& = & \dfrac{1}{4m}\cdot \delta \Big(\widetilde x \cup \widetilde x - \widetilde x \cup_1 \delta \widetilde x\Big)\\
		& = & \dfrac{1}{4m}\cdot\left[\Big(\delta \widetilde x \cup \widetilde x + \widetilde x \cup \delta \widetilde x\Big)
		...
		& = & \dfrac{1}{4m}\cdot\Big[2\cdot \delta \widetilde x \cup \widetilde x - \delta \widetilde x \cup_1 \delta \widetilde x\Big],\\
	\end{array}
\end{equation}
and thus if $x$ were a $\bZ_{2m}$-cocycle $\delta x = 0$ (mod $2m$)
and its lift $\widetilde x$ were $\delta \widetilde x = 0$ or $2m$ (mod $4m$),
then the right hand side would be zero (mod 1) and $\fP(x)$ would be a $\bZ_{4m}$-cocycle (i.e.~$0$ mod 4) as desired.
However, when $x$ is not a cocycle but merely a cochain, this term cannot be defined in the first place.

This can be saved somewhat miraculously as follows.
...
\begin{equation*}
	...
	0
\end{equation*}
buys us cohomology operations called higher Bockstein $\beta_f: H^{\bullet}(-;\bZ_{2^f}) \to H^{\bullet+1}(-;\bZ_{2^f})$,
and for the element $x \in C^{\bullet}(-;\bZ_{2^f})$ one has
\begin{equation*}
	\delta\widetilde x
	=
	2^f \beta_f (x).
...
Dividing the $SO\times SU$ by $\bZ_2$, these become non-closed
\begin{equation*}
...
Note that this can naively be regarded as $\tfrac{1}{2}\fP\big(w_2(c)\big)$ at the integral cochain level.
While the second term in the last line of \eqref{deltaBockstein} together with the overall factor takes value in
$\tfrac{(2m)^2}{4m}\bZ = m\bZ = 2\bZ$ and dividing by two does not cause any trouble,
the first term does as it takes value in $\tfrac{2\cdot 2m}{4m}\bZ = \bZ$.

Let us take a closer look at the latter.
The long exact sequence of cochain groups implies that $2\widetilde w_2(c) = v_2(c) = a_2$ [YL:make it more precise later].
Then the term of interest is
\begin{equation*}
	\dfrac{1}{8}\cdot 2\cdot 4\beta_2(a_2) \cup a_2.
\end{equation*}
