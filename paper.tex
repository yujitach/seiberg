...
This can be screened by a dynamical fermion, and that was why $W^2=1$ 
...
As we will recall below,  formally this means that the $\bZ_2$ 1-form symmetry extends the $\SU(2n_f)/\bZ_2$ 0-form symmetry in a nontrivial manner, forming a 2-group $H$\begin{equation}
...
and the $\SU(2n_f)/\bZ_2$ flavor 0-form symmetry are mixed nontrivially,
by examining the charges of line operators in each theory.
In Sec.~\ref{sec:sl2z},
we exploit the $\SL(2,\bZ_2)$ actions on $\so(2n_f)$ QCDs to determine the 't Hooft anomalies they possess.
...
so we partially complement it with the more direct computation of fermion anomalies in Sec.~\ref{sec:fermion}.
...
\end{equation} with the Postnikov class being $\beta a_2 \in H^3(B(\SU(2n_f)/\bZ_2);\bZ_2)$.
...
\end{equation}
where $w_2,a_2\in H^2(X;\bZ_2)$ controls the lift from $\SO(2n_c)/\bZ_2$ to $\SO(2n_c)$
